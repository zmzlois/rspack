...
use crate::visitors::{
  expr_matcher::{self, ExprLike},
  JavascriptParser, TopLevelScope,
};
...
impl<'parser> JavascriptParser<'parser> {
  fn is_exports_member_expr_start<E: ExprLike>(&mut self, expr: &E) -> bool {
    fn walk_each<'parser, E: ExprLike>(parser: &mut JavascriptParser<'parser>, expr: &E) -> bool {
      if parser.is_exports_expr(expr) {
        true
      } else if let Some(MemberExpr { obj, .. }) = expr.as_member() {
        walk_each(parser, (&**obj).into())
      } else {
        false
      }
    }
  ...
  fn is_this_member_expr_start<E: ExprLike>(&self, expr: &E) -> bool {
    ...
    fn walk_each<'parser, E: ExprLike>(parser: &JavascriptParser<'parser>, expr: &E) -> bool {
